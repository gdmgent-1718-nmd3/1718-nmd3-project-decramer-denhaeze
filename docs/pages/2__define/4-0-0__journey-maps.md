---
layout   : default
permalink: define/journey-maps/
published: true
# Custom Page Variables
# ─────────────────────
title: Journey Maps
---
## Maria Vanderbroecke

### Demografische gegevens

**Leeftijd:** 76 jaar   
**Job:** pensioen   
**Familie:** weduwe, 3 kinderen   
**Hobby's:** tijd doorbrengen met vriendinnen, museums bezoeken en breien   
**Interesses:** gezondheids weetjes, actualiteit   
**Woonplaats:** Merksem   
**Inkomen:** 1645 euro netto per maand   

### Digitaal profiel

**3 vaak bezochte websites**
- Facebook
- KMI
- Tripadvisor

**3 veel gebruikte apps**
- Messenger
- Buienalarm
- Candy Crush

**Toestellen**   
- Laptop
- Smartphone

### Lijst van taken

Het huishouden en op woensdag en zondag zorgen voor de kleinkinderen

### Frustraties
- Onvriendelijke mensen
- Als er geen mogelijkheid is om te rusten

### Algemene vragen
- Is die technologie wel nodig?

### Houding en overtuiging

Ze houdt van **nieuwe invloeden** en nieuwe dingen te leren om te zien. Ze gaat graag dingen doen met vriendinnen. Ze heeft geen haast en doet daarom alles op haar gemak. Ze is een levensgenieter met een open blik en **kijkt kritische** naar de dingen rondom haar

### Scenario

Maria is een vrouw van 76 jaar. Ze is **leergierig** en gaat daarom graag naar een museum met haar vriendinnen. Ze vind het belangrijk in haar uitstapjes dat ze dit op haar eigen tempo kan doen en eens kan **rusten**. Tijdens haar museum bezoek in het designmuseum in Gent kan ze de app gebruiken om te kijken **wanneer de bar open** is en wat je daar kan bestellen. Omdat ze een museum heeft overweldigend vind gebruikt ze de app om te kijken wat de **andere gebruikers goed vonden** om zo een selectie te maken van welke kamers ze gaat bekijken en welke niet. Ze kan ook kijken **hoe ze kan stappen** en wat er nog allemaal te zien is. Zo weet ze altijd waar ze naartoe moet en kan ze in alle rust genieten van het museum. Maar ze vind lezen heel vermoeiend en wil vooral kijken. Daarom gebruikt ze de app om voorwerpen die ze mooi vind of interessant **de bordjes te laten voorlezen of op te slaan**. Zo kan ze thuis deze informatie lezen en kan ze meer genieten in het museum zonder dat ze iets mist.

<table class="table table-striped">
  <tbody>
    <tr>
      <th scope="row">Fases:</th>
      <th>Naar het museum komen</th>
      <th>Ticket betalen</th>
      <th>Rondkijken</th>
      <th>Naar huis gaan </th>
    </tr>
    <tr>
      <th scope="row">Taken:</th>
      <td>
        <ul>
          <li>Route opzoeken</li> 
          <li>Openingsuren opzoeken </li>
          <li>Tentoonstelling bekijken</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Naar de kassa gaan </li>
          <li>Een ticket kopen </li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Bordjes lezen</li>
          <li>Objecten bekijken</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Route opzoeken </li>
        </ul>
      </td>
    </tr>
    <tr>
      <th scope="row">Gedachten:</th>
      <td>
        <ul>
          <li>Hoe gaan we naar het museum? </li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Hoeveel kost een ticket?</li>
          <li>Dit is lang aanschuiven </li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Waar moeten we naartoe?</li>
          <li>Zoveel lezen maakt me vermoeid</li>
          <li>Ik wil even rusten</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Frisse lucht en de rust doet me goed</li>
          <li>Het was interessant, maar ik ben heel moe </li>
        </ul>
      </td>
    </tr>
    <tr>
      <th scope="row">Emoties:</th>
      <td>
        <ul>
          <li>Afwachtend</li>
          <li>Nieuwsgierig </li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Vriendelijk personeel </li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Overweldigd</li>
          <li>Vermoeiend  </li>
          <li>Gedesoriënteerd </li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Opgelucht</li>
        </ul>
      </td>
    </tr>
    <tr>
      <th scope="row">Opportuniteiten:</th>
      <td>
        <ul>
          <li>De bezoekers worden de weg gewezen naar het museum</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>De tickets kunnen opvoorhand besteld worden</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Rustige ervaring creeëren</li>
          <li>Extra info bekijken via de app </li>
          <li>Wegwijzer </li>
          <li>Voorleesoptie </li>
          <li>Openingsuren van de bar </li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Ze kunnen de opgeslagen items bekijken</li>
        </ul>
      </td>
    </tr>
    <tr>
      <th scope="row">Pijnpunten:</th>
      <td>
        <ul>
          <li>Sommige bezoekers vinden het museum niet zo vlot</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Lange wachtrijen</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Geen duidelijk wegwijzers</li>
          <li>Heel veel lezen</li>
          <li>5 dagen in de week, geen mogelijkheid tot rusten.</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Sommige mensen vinden de weg naar de tram/bus of hun auto niet meer zo vlot terug</li>
        </ul>
      </td>
    </tr>
  </tbody>
</table>






<!--2e journey map-->





## Pieter Smit

### Demografische gegevens

**Leeftijd:** 32 jaar   
**Job:** Museumgids   
**Familie:** vrouw, 2 kinderen   
**Hobby's:** Basketbal, lezen, museums bezoeken   
**Interesses:** Geschiedenis, archeologie   
**Woonplaats:** Leiden, NL   
**Inkomen:** 1889 euro netto per maand   

### Digitaal profiel

**3 vaak bezochte websites**
- BN De Stem
- NMBS
- YouTube

**3 veel gebruikte apps**
- WhatsApp
- Spotify
- Candy Crush

**Toestellen**   
- Laptop
- Smartphone
- Smartwatch

### Lijst van taken

- Zorgen voor de kindjes
- Museumrondleidingen voorbereiden

### Frustraties
- Lange reistijden met het openbaar vervoer
- Zaken in het museum waar geen bordje met uitleg bij staat
- Onvriendelijke mensen

### Algemene vragen
- Waarom kunnen Belgen geen wegen bouwen?


### Scenario

Pieter Smit is een 32-jarige, trotse, getrouwde man en de vader van twee kinderen. Hij verdient zijn boterham als **museumgids** en gaat regelmatig zelf museums bezoeken **in verschillende landen**, tot wel 40 museums op een jaar tijd. Onze app zal hij enkel gebruiken indien er geen info in het museum zelf geplaatst is of achteraf thuis om bepaalde zaken nog eens te bekijken of na te lezen. Tijdens het museumbezoek wil hij zijn **aandacht niet laten afleiden door alle digitale toestellen** in zijn omgeving.

<table class="table table-striped">
  <tbody>
    <tr>
      <th scope="row">Fases:</th>
      <th>Naar het museum komen</th>
      <th>Ticket betalen</th>
      <th>Rondkijken</th>
      <th>Naar huis gaan </th>
    </tr>
    <tr>
      <th scope="row">Taken:</th>
      <td>
        <ul>
          <li>Adres opzoeken</li> 
          <li>Openingsuren opzoeken</li>
          <li>Uren opzoeken voor het openbaar vervoer</li>
          <li>Bekijken welke tentoonstellingen er zijn</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Naar de kassa gaan</li>
          <li>Ticket betalen</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Bordjes lezen</li>
          <li>Objecten bekijken</li>
          <li>De weg vinden doorheen het museum</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Uren opzoeken voor het openbaar vervoer</li>
          <li>Op tijd vertrekken richting de bushalte</li>
        </ul>
      </td>
    </tr>
    <tr>
      <th scope="row">Gedachten:</th>
      <td>
        <ul>
          <li>In welke richting moet ik lopen als ik de bus uitstap?</li>
          <li>Wat is het mooi weer</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Waar is de kassa?</li>
          <li>Hoeveel kost een ticket?</li>
          <li>Hoeveel zal dit mij kosten als ik met mijn gezin langskom?</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Ik vind het zo nutteloos als er een voorwerp staat zonder bordje</li>
          <li>Ben ik al in deze zaal geweest?</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>In het algemeen was het wel een interessant museum</li>
          <li>Ik moet snel naar de bushalte teruggaan vooralleer ik mijn trein mis</li>
        </ul>
      </td>
    </tr>
    <tr>
      <th scope="row">Emoties:</th>
      <td>
        <ul>
          <li>Benieuwd</li>
          <li>Nieuwsgierig</li>
          <li>Verveeld door het slechte openbaar vervoer</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Verward door de vele prijzen</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Geboeid</li>
          <li>Geërgerd door de objecten zonder uitleg</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Gehaast</li>
          <li>Tevreden</li>
        </ul>
      </td>
    </tr>
    <tr>
      <th scope="row">Opportuniteiten:</th>
      <td>
        <ul>
          <li>Duidelijke overzicht van de verschillende tentoonstellingen met korte info</li>
          <li>Mogelijkheid om van de bushalte naar het museum geleid te worden</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Tickets op voorhand bestellen via site of app </li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Extra info via een andere manier dan bordjes indien er geen bordjes zijn</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Onderweg naar huis nog even de informatie herbekijken</li>
        </ul>
      </td>
    </tr>
    <tr>
      <th scope="row">Pijnpunten:</th>
      <td>
        <ul>
          <li>De weg van de bushalte naar het museum is niet zo duidelijk</li>
          <li>Niet zo'n goede verbindingen met het openbaar vervoer</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>Er zijn zoveel verschillende prijzen waardoor je niet snel prijs vindt die je moet betalen.</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>De weg doorheen het museum is verwarrend</li>
          <li>Niet elk voorwerp heeft een bordje met uitleg</li>
          <li>De uitgang van het museum vinden is moeilijk als je ergens in één van de gangen zit</li>
        </ul>
      </td>
      <td>
        <ul>
          <li>De bus die je moet hebben om de juiste treinverbinding te hebben, komt 1,5uur voor de sluitingstijd van het museum.</li>
        </ul>
      </td>
    </tr>
  </tbody>
</table>